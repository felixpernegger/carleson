...
A second question is whether the sequence $\{ S_N f (x)\}_{\{ N \in\Nat\}}$ converges pointwise to $f(x)$ for almost every $x$. This turned out to be a significantly harder problem.
...
\begin{theorem}[classical Carleson]
    ...
    Let $f$ be a $2\pi$-periodic complex-valued continuous function on $\real$.
    Then for almost all $x \in \real$, the limit
...
\end{equation}
with $\kappa\colon \real\to\C$ defined as
\begin{equation}
...
Let $a \ge 4$ be a natural number. A collection $\Mf$ of real-valued continuous functions on the doubling metric measure space $(X,\rho,\mu,a)$ is called compatible,
...
is a measurable function $K\colon X\times X\to \complex$ such that for all $x,y',y\in X$ with $x\neq y$, we have
...
$[\frac1{2D},\frac 14]$, and is Lipschitz with constant $4D$. With this definition, the finitary Carleson Theorem reads as follows.
...
for all Borel functions $f\colon X\to \C$ with $|f|\le \mathbf{1}_F$ we have
\begin{equation*}
...
sets $I$ in $X$ and integers $k \in [-S, S]$, the scale function $s\colon \mathcal{D}\to [-S, S], (I, k) \mapsto k$, assumed to be surjective, and a function $c:\mathcal{D}\to X$
...
\begin{equation}\label{dyadicproperty}
    I\subset J.
\end{equation}
There exists an $I_0 \in \mathcal{D}$ with $s(I_0) = S$ and $c(I_0) = o$
...
The proof of this last result is the heart of the matter.
% todo: JP: I think the part below should be clarified
The pieces are categorized in one of three types, for each of which a separate analysis is required to establish a bound.
...
This preprint consisted of a 30-page proof of Theorem \ref{metric-space-Carleson}. %todo: double check.
